...
def negamax_decision(state, depth):
	...
	for move in range(1, 4):
		if state - move > 0:
			_, utility = negamax_decision(state - move, depth - 1)
			new_utility = -utility  # negate the utility
...
def play_nim(state):
	...
	while state != 1:
		...
		move, value = negamax_decision(state, depth)
